...
do ffmpeg -y -i "$i" -vcodec libwebp -filter:v fps=fps=20 -lossless 0  -compression_level 3 -q:v 70 -loop 0 -preset picture -an -vsync 0 -filter:v scale=1280:-1 "${i%.*}.webp"
